...
$(function () {
    ...
    $("#action").click(function() {
        var isCompleted = $('#completed')[0].checked;
        var name = $('#task')[0].value;

        tasksManager.createTask(isCompleted, name)
            .then(tasksManager.loadTasks)
            .done(function(tasks) {
                tasksManager.displayTasks("#new", tasks);
            });
    });

    // bind update task checkbox click handler
    $("#new").on('change', '.completed', function () {
    ...
    $('#new').on('click', '.delete-button', function() {
